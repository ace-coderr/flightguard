...
import { buildFlightRequestBody, computeRequestHash, parseFlightRef } from "./flightRequest";
...
async function getDuePolicies(publicClient: PublicClient): Promise<DuePolicy[]> {
  ...
  return rawPolicies
    .map(([, , , scheduledArrival, requestHash, flightRef, status], id): DuePolicy & { status: number } => ({
      id,
...
async function settleOnePolicy(
  ...
  try {
    ...
    if (requestHash !== policy.requestHash) {
    ...
    const { roundId, abiEncodedRequest } = await startSettleJob(flightIata, date);
...
export async function runKeeperTick(deadline: number): Promise<KeeperTickResult> {
  ...
  console.log(`[keeper] ${due.length} due polic${due.length === 1 ? "y" : "ies"}: ${due.map((p) => p.id).join(", ")}`);
